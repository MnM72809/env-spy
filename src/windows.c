...
AppWindows create_windows()
{
	...
	int left_pane_height =
	    term_y - status_bar_height - 6; // Leave space for status bar
	...
	WINDOW *filter_bar_win =
	    newwin(filter_bar_height, filter_bar_width, title_height, 0);
	WINDOW *left_pane_win =
	    newwin(left_pane_height, left_pane_width, left_pane_top, 0);
	WINDOW *right_pane_win = newwin(right_pane_height, right_pane_width,
	                                right_pane_top, left_pane_width);
	WINDOW *status_bar_win = newwin(status_bar_height, status_bar_width,
	                                term_y - status_bar_height, 0);

	// Create inner windows for text printing
	WINDOW *left_inner_win =
	    derwin(left_pane_win, left_pane_height - 2, left_pane_width - 2, 1, 1);
	WINDOW *right_inner_win = derwin(right_pane_win, right_pane_height - 2,
	                                 right_pane_width - 2, 1, 1);
	...
	AppWindows app_windows = {.title_win = title_win,
